        ...
        <div class="flex items-center gap-4 border-t py-6">
            @if ($this->getCurrentTheme() instanceof \Hasnayeen\Themes\Contracts\HasChangeableColor)
                ...
                <div class="flex items-center space-x-4 rtl:space-x-reverse">
                    ...
                    <label for="custom">{{ __('themes.custom') }}</label>
                </div>
            @else
                <p class="text-gray-700 dark:text-gray-400">{{ __('themes.no_changing_primary_color') }}</p>
            @endif
        </div>
    </section>
            ...
            @foreach ($this->getThemes() as $name => $theme)
                ...
                <x-filament::section>
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            @if ($supportColorChange)
                                <span
                                    x-data="{}"
                                    x-tooltip="{
                                        content: '{{ __('themes.support_changing_primary_color') }}',
                                        theme: $store.theme,
                                    }"
                            ...
                            @if (! $noLightMode)
                                <span
                                    x-data="{}"
                                    x-tooltip="{
                                        content: '{{ __('themes.support_light_mode') }}',
                                        theme: $store.theme,
                                    }"
                                    class="bg-primary-200 flex items-center justify-center p-1 rounded-full">
                                    <svg class="w-4 h-4 dark:text-gray-800" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            ...
                            @if (! $noDarkMode)
                                <span
                                    x-data="{}"
                                    x-tooltip="{
                                        content: '{{ __('themes.support_dark_mode') }}',
                                        theme: $store.theme,
                                    }"
                                    class="bg-primary-200 flex items-center justify-center p-1 rounded-full">
                                    <svg class="w-4 h-4 dark:text-gray-800" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            ...
                            @if ($this->getCurrentTheme()->getName() === $name)
                                <span
                                    x-data="{}"
                                    x-tooltip="{
                                        content: '{{ __('themes.theme_active') }}',
                                        theme: $store.theme,
                                    }"
                                    class="bg-primary-200 flex items-center justify-center p-1 rounded-full">
                                    <svg class="w-4 h-4 dark:text-gray-800" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            @if ($noLightMode)
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">{{ __('themes.no_light_mode') }}</h3>
                            @else
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">{{ __('themes.light') }}</h3>
                        ...
                        <div>
                            @if ($noDarkMode)
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">{{ __('themes.no_dark_mode') }}</h3>
                            @else
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">{{ __('themes.dark') }}</h3>
