...
class ActionHelper
{
    public static function makeSlideOver(
        ...
        ?callable $after = null,
        bool|callable $visible = true,
        bool|callable $clickble = true
    ): Action {
        $isEdit = $label === 'Editar';
        ...
        if(!$visible) {
            $actionBuilder->extraAttributes(['style' => 'display:none;']);
        }
    ...
    public static function makeSlideOverView(
        string $name,
        array $form,
        string $modalHeading,
        string $label = 'Editar',
        ...
        $actionBuilder = ActionAction::make($name)
            ->form($form)
            ->model('formData')
            ->modalHeading($modalHeading)
            ->modalButton($isEdit ? 'Salvar alterações' : 'Salvar')
